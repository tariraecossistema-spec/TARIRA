      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-2xl w-full border border-border shadow-2xl relative my-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              <label className="text-xs text-slate-700 block font-semibold mb-1">
                {formType === 'provider' ? 'Taxa Base (MZN / Hora)' : 'Pretensão Mensal (MZN)'}
              </label>
              <input
                type="number"
                value={formType === 'provider' ? hourlyRate : expectedSalaryMin}
                onChange={(e) => {
                  if (formType === 'provider') {
                    setHourlyRate(Number(e.target.value));
                  } else {
                    setExpectedSalaryMin(Number(e.target.value));
                  }
                }}
